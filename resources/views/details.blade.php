                ...
                <div class="swiper-wrapper">
                  ...
                  @if($product->images)
                    @foreach(explode(',', $product->images) as $img)
                      ...
                      ?>
                      <div class="swiper-slide product-single__image-item">
                        <img loading="lazy" class="h-auto" src="{{ asset('uploads/products') }}/{{ trim($cleanImgName) }}" width="674"
                          height="674" alt="" />
                        <a data-fancybox="gallery" href="{{ asset('uploads/products') }}/{{ trim($cleanImgName) }}" data-bs-toggle="tooltip"
                   ...
                   @if($product->images)
                      @foreach(explode(',', $product->images) as $img)
                        <?php 
                          $cleanImgName = str_replace(['[', ']', '"'], '', $img);
                        ?>
                        <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('uploads/products/thumbnails') }}/{{ trim($cleanImgName) }}" width="104" height="104" alt="" /></div>
                      @endforeach
                    @endif
                </div>
              </div>
          ...
          <div class="product-single__addtolinks">
            @if (Cart::instance("wishlist")->content()->where('id', $product->id)->count()>0)
              <form action="{{ route('wishlist.item.remove', ['rowId' => Cart::instance("wishlist")->content()->where('id', $product->id)->first()->rowId]) }}" method="POST" id="remove-item">
                @csrf
                @method('DELETE')
              <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart" onclick="document.getElementById('remove-item').submit();"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
            ...
            @foreach ($related_products as $related_product)
              <div class="swiper-slide product-card">
                <div class="pc__img-wrapper">
                  <a href="{{ route('shop.product.details', ['productSlug'=>$related_product->slug]) }}">
                    <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $related_product->image }}" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img">
                    @if($related_product->images)
                      @foreach(explode(',', $related_product->images) as $img)
                        <?php 
                          $cleanImgName = str_replace(['[', ']', '"'], '', $img);
                        ?>
                        <img loading="lazy" src="{{ asset('uploads/products') }}/{{ trim($cleanImgName) }}" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img pc__img-second">
                      @endforeach
                    @endif
                  </a>
                  @if(Cart::instance('cart')->content()->where('id', $product->id)->count()>0)
                ...
                <div class="pc__info position-relative">
                  ...
                  <h6 class="pc__title"><a href="{{ route('shop.product.details', ['productSlug'=>$product->slug]) }}">{{ $related_product->name }}</a></h6>
